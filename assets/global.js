...
window.addEventListener('load', function () {
    ...
    const prevButton = document.querySelector('.slideshow__arrow--left');
    const nextButton = document.querySelector('.slideshow__arrow--right');
    const flickityElement = slideshow.querySelector('.flickity-enabled');

    if (!flickityElement || !prevButton || !nextButton) return;

    // Aguarda o Flickity ser carregado corretamente antes de vincular os eventos
    const checkFlickity = setInterval(() => {
        const flickityInstance = Flickity.data(flickityElement);
        if (flickityInstance) {
...
theme.Currency = (function() {
  ...
  function toggleDescription() {
    ...
    var text = description.querySelector('.description-text');
    var toggleLink = description.querySelector('.toggle-description');
    
    if (text.style.webkitLineClamp === "none") {
      text.style.webkitLineClamp = 2; // Limita para 2 linhas
      toggleLink.textContent = "Ver Mais...";
    } else {
      text.style.webkitLineClamp = "none"; // Expande o conteúdo
